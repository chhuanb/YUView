...
MoveAndZoomableView::MoveAndZoomableView(QWidget *parent) : QWidget(parent)
{
...
bool MoveAndZoomableView::handleKeyPress(QKeyEvent *event)
{
  ...
  {
    // This seems to be a bug in the Qt localization routine. On the German keyboard layout this key
    // is returned if Ctrl + is pressed.
    this->zoom(ZoomMode::IN);
    return true;
  }
...
void MoveAndZoomableView::updatePaletteIfNeeded()
{
  if (this->paletteNeedsUpdate)
  {
    ...
    pal.setColor(QPalette::Window,
                 settings.value(paletteBackgroundColorSettingsTag).value<QColor>());
 ...
 *
 * Zooming is performed in steps of ZOOM_STEP_FACTOR. However, the zoom factor can not be a multiple
 * of ZOOM_STEP_FACTOR in two cases: The used pinched the screen or entered a custom zoom value. If
 * this is the case and a normal zoom operation happens, we "snap back" to a multiple of
 * ZOOM_STEP_FACTOR. E.g.: If the zoom factor currently is 1.9 and we zoom in, it will become 2.
 *
 * \param zoomMode In/out or to a specific value. See ZoomMode.
 * \param zoomPoint After the zoom operation the pixel at zoomPoint shall still be at the same
 * position (zoomPoint) \param newZoomFactor Zoom to this value in case of ZoomMode::TO_VALUE
 */
void MoveAndZoomableView::zoom(MoveAndZoomableView::ZoomMode zoomMode,
                               QPoint                        zoomPoint,
                               double                        newZoomFactor)
{
  ...
  DEBUG_VIEW("MoveAndZoomableView::zoom point debug zoomPoint "
             << zoomPoint << " viewCenter " << viewCenter << " this->moveOffset "
             << this->moveOffset << " centerMoveOffset " << centerMoveOffset << " stepZoomFactor "
             << stepZoomFactor << " movementDelta " << movementDelta);
...
void MoveAndZoomableView::mouseMoveEvent(QMouseEvent *mouse_event)
{
  ...
  {
    // The mouse is moved, but no button is pressed. This should not be caught here. Maybe a mouse
    // press/release event got lost somewhere. In this case go to the normal mode.
    if (this->viewAction == ViewAction::DRAGGING ||
        this->viewAction == ViewAction::DRAGGING_MOUSE_MOVED ||
      ...
      this->viewAction == ViewAction::DRAGGING_MOUSE_MOVED)
  {
    ...
    this->setMoveOffset(viewDraggingStartOffset +
...
void MoveAndZoomableView::mousePressEvent(QMouseEvent *mouse_event)
{
      ...
      (mouse_event->button() == Qt::RightButton && mouseMode == MOUSE_RIGHT_MOVE))
  {
    ...
    DEBUG_VIEW("MoveAndZoomableView::mousePressEvent start dragging posStart "
               << this->viewDraggingMousePosStart << " startOffset "
               << this->viewDraggingStartOffset);
    mouse_event->accept();
  }
           ...
           (mouse_event->button() == Qt::LeftButton && mouseMode == MOUSE_RIGHT_MOVE))
  {
    ...
    DEBUG_VIEW("MoveAndZoomableView::mousePressEvent start zoombox posStart "
               << this->viewZoomingMousePosStart << " mousePos " << this->viewZoomingMousePos);
    mouse_event->accept();
  }
...
{
  // IMPORTANT:
  // We are not using the QPanGesture as this uses two fingers. This is not documented in the Qt
  // documentation.
  ...
  if (event->type() == QEvent::Gesture)
  {
    ...
    if (QGesture *swipeGesture = gestureEvent->gesture(Qt::SwipeGesture))
    {
      ...
      {
        if (swipe->horizontalDirection() == QSwipeGesture::NoDirection &&
            swipe->verticalDirection() == QSwipeGesture::Up)
          this->onSwipeUp();
        else if (swipe->horizontalDirection() == QSwipeGesture::NoDirection &&
                 swipe->verticalDirection() == QSwipeGesture::Down)
          this->onSwipeDown();
        else if (swipe->horizontalDirection() == QSwipeGesture::Left &&
                 swipe->verticalDirection() == QSwipeGesture::NoDirection)
          this->onSwipeLeft();
    ...
    if (auto pinchGesture = gestureEvent->gesture(Qt::PinchGesture))
    {
      ...
      if (pinch->state() == Qt::GestureStarted || pinch->state() == Qt::GestureUpdated)
      {
        ...
        if (changeFlags & QPinchGesture::ScaleFactorChanged ||
            changeFlags & QPinchGesture::CenterPointChanged)
        {
          if (newZoom < ZOOMINGLIMIT.min || newZoom > ZOOMINGLIMIT.max)
            doZoom = false;
          else
          {
            ...
            setMoveOffset((this->pinchStartMoveOffset * pinch->totalScaleFactor() +
                           pinch->centerPoint() - this->pinchStartCenterPoint)
                              .toPoint());
          }
        }
      }
  ...
  else if (isTouchScreenEvent)
  {
    ...
    if (touchPoints.size() >= 1)
    {
      ...
      if (this->viewAction == ViewAction::NONE && event->type() == QEvent::TouchBegin)
      {
        ...
        DEBUG_VIEW("MoveAndZoomableView::event start touch dragging posStart "
                   << this->viewDraggingMousePosStart << " startOffset "
                   << this->viewDraggingStartOffset);
      }
...
void MoveAndZoomableView::zoomToCustom(bool)
{
  ...
  int  newValue = QInputDialog::getInt(this,
                                      "Zoom to custom value",
                                      "Please select a zoom factor in percent",
                                      100,
                                      1,
                                      2147483647,
                                      1,
...
void MoveAndZoomableView::setLinkState(bool enabled)
{
  ...
  else
  {
    ...
    DEBUG_VIEW("MoveAndZoomableView::setLinkState slave send link state " << enabled
                                                                          << " to master");
