 ...
 */
inline int transformYUV(
  const bool invert, const int scale, const int offset, const unsigned int value, const int clipMax)
{
  int newValue = value;
  if (invert)
...
QLayout *videoHandlerYUV::createVideoHandlerControls(bool isSizeAndFormatFixed)
{
  ...
  connect(ui.lumaOffsetSpinBox,
          ...
          &videoHandlerYUV::slotYUVControlChanged);
  connect(
    ui.lumaInvertCheckBox, QCheckBoxStateChanged, this, &videoHandlerYUV::slotYUVControlChanged);
  ...
  connect(ui.chromaOffsetSpinBox,
          ...
          &videoHandlerYUV::slotYUVControlChanged);
  connect(
    ui.chromaInvertCheckBox, QCheckBoxStateChanged, this, &videoHandlerYUV::slotYUVControlChanged);
                                            ...
                                            const bool       markDifference)
{
  ...
  const unsigned componentSizeChroma_In[2] = {chromaPresent ? (w_in[0] / subH) * (h_in[0] / subV) : 0,
                                              chromaPresent ? (w_in[1] / subH) * (h_in[1] / subV) : 0};
  ...
  const int componentSizeChroma_out = chromaPresent ? (w_out / subH) * (h_out / subV) * (bps_out > 8 ? 2 : 1) : 0;
  ...
  // Next U/V (only if chroma is present)
  if (chromaPresent)
  {
